...
ints = unsafePerformIO $ readFile "cipher1.txt" >>= \s -> 
        return $ map read $ words $ map (\c -> if c == ',' then ' ' else c) s 

ls = enumFromTo 'a' 'z'
letters = ls ++ ls ++ ls ++ enumFromTo '0' '9' 
keys = combinations 3 letters

unencrypt key = map chr $ zipWith xor ints rkey where
    rkey = map ord $ cycle key

answers = [x | x <- map unencrypt keys, hasCommonWords x] where
    hasCommonWords s = let ws = (words $ map toLower s) in (elem "and" ws) && (elem "the" ws)

text = head answers
answer = sum $ map ord text
